...
void function () {
 ...
 smoke.zindex           = 9000         // Z-index of the smoke DOM object. This should be a high number.
...
// -------------  мой код ------------------------------------------------------------------------------------
var blackBox = obj.blackBox = document.createElement ('div');
  blackBox.className = css_prefix + '-black-box'
  obj.appendChild (blackBox)

   setTimeout(() => {
  	obj.classList.add('smoke-base-visible');
  	blackBox.classList.add('smoke-black-box-visible');
   },1);
// -------------  мой код -------------------------------------------------------------------------------------
  ...
  if (params.type == 'prompt') {
   ...
   prompt.input = document.createElement ('input'); prompt.input.type = 'text';  prompt.input.className = css_prefix + '-prompt-input'; prompt.input.placeholder = 'Введите имя'
...
// ------------ мой код----------------------------------------------------------  
  ...
  if (typeof obj.prompt != "undefined") {
	obj.prompt.input.oninput = function() {
		if(regexp.test(this.value)) {
			obj.prompt.input.style.borderBottom = '1px red solid'
	  		setTimeout(() => obj.prompt.input.style.borderBottom = '1px #aaa solid',100)
		}
...
//------------------------------------------------------------------------------
     ...
     for (var i = 0, curlen_i = mutation_list.length; i < curlen_i; i++) {
      ...
      for (var j = 0, curlen_j = mutation_item.removedNodes.length; j < curlen_j; j++) {
       ...
       if ((!window_closed_ran) && smoke.window_closed) {
       	smoke.window_closed (obj, text, params); window_closed_ran = true}	
 ...
 //-----------------------------------мой код--------------------------------------------------------------------
  ...
  if(obj.prompt) {
  	...
  	if(obj.prompt.input.value.trim() ==='' || obj.prompt.input.value.trim().length > 20) {
  		obj.prompt.input.style.borderBottom = '1px red solid'
  		setTimeout(() => obj.prompt.input.style.borderBottom = '1px #aaa solid',100)
  		return
  	}
